        ...
        if serializer.is_valid():
            ...
            if values_args:
                if len(values_args) == 1:
                    ...
                    queryset = (
                        queryset.values_list(*values_args_query_set).order_by(values_args_query_list[0]).distinct()
                    )
...
class RequestIdLimsPullViewSet(GenericAPIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"details": "Import requests from LIMS jobs submitted %s" % str(request_ids)},
