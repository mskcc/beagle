...
def create_request_job(request_id, redelivery=False):
    logger.info("Searching for job: %s for request_id: %s" % (TYPES["REQUEST"], request_id))
    ...
    if (
        not all(
            [
                ...
                for job in Job.objects.filter(job_group=job_group, run=TYPES["SAMPLE"], args__igocomplete=True).values(
                    "status"
                )
            ]
        )
...
def get_or_create_pooled_normal_job(filepath, job_group=None, job_group_notifier=None, redelivery=False):
    logger.info("Searching for job: %s for filepath: %s" % (TYPES["POOLED_NORMAL"], filepath))
...
def create_sample_job(
    sample_id, igocomplete, request_id, request_metadata, redelivery=False, job_group=None, job_group_notifier=None
):
    job = Job(
        run=TYPES["SAMPLE"],
        args={
...
def create_pooled_normal(filepath, file_group_id):
    ...
    if None in [run_id, preservation_type, recipe]:
        ...
        return
    metadata = {"runId": run_id, "preservation": preservation_type, "recipe": recipe}
    try:
...
def fetch_sample_metadata(
    sample_id, igocomplete, request_id, request_metadata, redelivery=False, job_group_notifier=None
):
        ...
        if not f:
            ...
            if update:
                ...
                if after != before:
                    ...
                    ddiff = DeepDiff(all_metadata[1].metadata, all_metadata[0].metadata, ignore_order=True)
