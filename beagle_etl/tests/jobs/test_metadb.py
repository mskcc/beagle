...
class TestNewRequest(TestCase):
    ...
    def setUp(self):
        self.request_keys = ["piEmail", "projectManagerName", "labHeadName", "labHeadEmail", "investigatorName", "investigatorEmail", "dataAnalystName", "dataAnalystEmail", "otherContactEmails", "dataAccessEmails", "qcAccessEmails", settings.REQUEST_ID_METADATA_KEY,
        ...
        test_files_fixture = os.path.join(
            settings.TEST_FIXTURE_DIR, "10075_D_2.filemetadata.json")
        call_command("loaddata", test_files_fixture, verbosity=0)
        new_request_json_path = os.path.join(
            settings.TEST_FIXTURE_DIR, "10075_D_2.update.json")
    ...
    def test_update_request_metadata(self, request_callback, populate_job_group, send_notification, jobGroupNotifierObjectGet):
        """
        ...
        files = FileRepository.filter(
            metadata={settings.REQUEST_ID_METADATA_KEY: "10075_D_2"})
        for file in files:
            for single_request_key in self.request_keys:
                self.assertEqual(
                    file.metadata[single_request_key], self.new_request_data[single_request_key])
    ...
    def test_update_request_ticket(self, ticket_description, request_callback, populate_job_group, send_notification, jobGroupNotifierObjectGet):
        """
        ...
        for single_request_key in self.request_keys:
            self.assertEqual(
                request_metadata[single_request_key], self.new_request_data[single_request_key])

    @patch("notifier.models.JobGroupNotifier.objects.get")
    @patch("notifier.tasks.send_notification.delay")
    @patch("file_system.tasks.populate_job_group_notifier_metadata.delay")
    @patch("beagle_etl.jobs.metadb_jobs.request_callback")
    def test_update_request_sample(self, request_callback, populate_job_group, send_notification, jobGroupNotifierObjectGet):
        """
    ...
    def test_update_sample_update(self, populate_job_group, send_notification, jobGroupNotifierObjectGet):
        ...
        send_notification.return_value = None
        sample_metadata = {}
        file = FileRepository.filter(
            metadata={settings.REQUEST_ID_METADATA_KEY: "10075_D_2"}).first()
