...
class FileTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.file_type_bam = FileType(ext='bam')
        self.file_type_bam.save()

    def _create_single_file(self, path, file_type, group_id, request_id, sample_id):
        self.client.credentials(HTTP_AUTHORIZATION='JWT %s' % self._generate_jwt())
    ...
    def test_create_file_unauthorized(self):
        self.client.credentials(HTTP_AUTHORIZATION='JWT %s' % "invalid_token")
        response = self.client.post('/v0/fs/files/',
                                    {
                                        "path": "/path/to/file.fasta",
                                        "size": 1234,
                                        "file_group": str(self.file_group.id),
                                        "file_type": str(self.file_type_fasta.ext)
                                    },
                                    format='json')
        ...
        response = self.client.post('/v0/fs/files/',
                                    {
                                        ...
                                        "file_type": self.file_type_fasta.ext,
                                        "metadata": {
        ...
        response = self.client.put('/v0/fs/files/%s/' % _file['id'],
                                   {
                                       ...
                                       "size": _file['size'],
                                       "file_group_id": _file['file_group']['id'],
                                       "file_type_id": _file['file_type'],
                                       "metadata": {
                                           "requestId": "Request_001",
                                           "igoSampleId": _file['metadata']['igoSampleId']
                                       }
                                   },
                                   format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['metadata']['requestId'], "Request_001")
        file_metadata_count = FileMetadata.objects.filter(file=_file['id']).count()
    ...
    def test_update_file_metadata_users_update(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION='JWT %s' % self._generate_jwt(username=user1.username))
        response = self.client.put('/v0/fs/files/%s/' % _file['id'],
                                   {
                                       "path": _file['file_name'],
                                       "size": _file['size'],
        ...
        self.assertEqual(response.data['metadata']['requestId'], 'Request_001')
        self.client.credentials(HTTP_AUTHORIZATION='JWT %s' % self._generate_jwt(username=user2.username))
        response = self.client.put('/v0/fs/files/%s/' % _file['id'],
                                   {
                                       "path": _file['file_name'],
                                       "size": _file['size'],
    ...
    def test_delete_file(self):
        _file = self._create_single_file('/path/to/sample_file.bam',
                                         ...
                                         'sample_id')
        self.client.credentials(HTTP_AUTHORIZATION='JWT %s' % self._generate_jwt())
        response = self.client.delete('/v0/fs/files/%s/' % _file['id'])
    ...
    def test_list_all_metadata(self):
        ...
        response = self.client.put('/v0/fs/files/%s/' % _file['id'],
                                   ...
                                   format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.get('/v0/fs/metadata/?file_id=%s' % _file['id'], format='json')
