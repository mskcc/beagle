    ...
    for single_run_id in argos_run_id_list:
        ...
        for single_port in port_list:
            ...
            if name == "facets_txt_hisens":
                input_json["facets_hisens_cncf_files"] = input_json["facets_hisens_cncf_files"] + get_files_from_port(
                    value
                )
            if name == "facets_seg":
                input_json["facets_hisens_seg_files"] = input_json["facets_hisens_seg_files"] + get_files_from_port(
                    value
                )
...
def get_request_pi(run_id_list):
    ...
    for request_id in all_request_ids:
        investigator_emails = FileRepository.filter(queryset=files, metadata={settings.REQUEST_ID_METADATA_KEY: request_id}).values_list('metadata__investigatorEmail', flat=True)
