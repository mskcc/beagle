...
from .copy_outputs_operator import construct_copy_outputs_input
...
class CopyOutputsOperator(Operator):
    def get_jobs(self):
        ...
        name = "ROSLIN COPY OUTPUTS %s runs [%s,..] " % (number_of_runs,run_ids[0])
        copy_outputs_job_data = {'app': self.get_pipeline_id(), 'inputs': input_json, 'name': name}
