...
class AccessHemeLegacySVOperator(Operator):
    """
    ...
    def get_sample_inputs(self, runs):
        ...
        for i, b in enumerate(standard_tumor_bams):
            sample_input = self.construct_sample_inputs(sample_ids[i], b)
    ...
    def get_jobs(self):
        ...
        return [
            RunCreator(
                **{
                    "name": "ACCESS HEME LEGACY SV M1: %s, %i of %i" % (self.request_id, i + 1, len(sample_inputs)),
                    "app": self.get_pipeline_id(),
                    "inputs": job,
                    "tags": {
