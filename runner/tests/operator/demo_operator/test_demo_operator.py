...
class TestDemoOperator(TestCase):
    ...
    def test_live_demo_operator_run(self):
        ...
        if enable_testcase:
            ...
            filemetadata_instance = FileMetadata.objects.create(file=file_instance, metadata={settings.REQUEST_ID_METADATA_KEY: request_id})
            ...
            request = MockRequest()
            request.data = {"request_ids": [request_id], "pipeline_name": DemoOperator._pipeline_name}
            view = OperatorViewSet()
            response = view.post(request)
