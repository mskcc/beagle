...
class PortViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, mixins.UpdateModelMixin, GenericViewSet):
        ...
        if isinstance(port.schema.get("type"), dict):
            ...
            if port.schema.get("type") != "File":
                ...
                try:
                    ...
                    return Response({"details": "Not Found"}, status=status.HTTP_404_NOT_FOUND)
                port.value = {
                    "inputs": self._create_file(file, port.schema.get("secondaryFiles")),
                    "refs": str(file.id),
                }
        port.save()
