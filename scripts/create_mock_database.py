...
def create_sample_file_metadata(
    faker,
    file,
    user,
    R,
    ...
    file_metadata = {
        ...
        "flowCellLanes": [randint(2, 10)],
        "libraryVolume": None,
        "oldSampleName": sampleID,
        "sampleAliases": [
            {"value": libraryId, "namespace": "igoId"},
            {"value": sampleName, "namespace": "investigatorId"},
        ],
        ...
        "patientAliases": [{"value": patientName, "namespace": "cmoId"}],
        "qcAccessEmails": "",
    ...
    while count <= num_samples:
        for r in ["R1", "R2"]:
            (
                ...
                sample_origin,
                oncoTree_code,
            ) = get_sample_info(faker, patient, count, "T", request)
        ...
        if faker.boolean(chance_of_getting_true=pair_percent):
            for r in ["R1", "R2"]:
                (
                    ...
                    sample_origin,
                    oncoTree_code,
                ) = get_sample_info(faker, patient, count, "N", request)
                path = faker.file_path(depth=randint(3, 5), extension="fastq")
                file_name = os.path.basename(path)
                sample_file = File(file_name=file_name, path=path, file_type=file_type, file_group=file_group, size=0)
                sample_list.append(sample_file)
...
def create_patient(faker, user, file_group, request, genePanel, investigator, lab_head, data_analyst, num_patients):
    ...
    while count <= num_patients:
        num_normals = randint(2, 5)
        patient = "C-{}".format(uuid.uuid4().hex[:6])
        sex = faker.male_or_female()
        sample_list, metadata_list = create_sample(
            faker,
            user,
            file_group,
            request,
...
def create_request(faker, num_requests, user):
    ...
    while count <= num_requests:
        ...
        samples_list, metadata_list = create_patient(
            faker, user, file_group, request, genePanel, investigator, lab_head, data_analyst, randint(1, 5)
        )
        count += 1
...
def create_qc_run(
    faker, user, first_file_list, second_file_list, status, operator_run, job_group, request_id, start_date, end_date
):
    normal_sample_list = []
    tumor_sample_list = []
    ...
    tags = {
        ...
        "sampleNameNormal": normal_sample_list,
        "sampleNameTumor": tumor_sample_list,
    }
...
def create_run_from_file(
    faker, user, first_file, second_file, status, operator_run, job_group, request_id, num, total, start_date, end_date
):
    ...
    tags = {
        ...
        "igoRequestId": request_id,
        "labHeadEmail": lab_head_email,
        "sampleNameNormal": sample_name_normal,
        "sampleNameTumor": sample_name_tumor,
    }
    ...
    if status == RunStatus.COMPLETED:
        file_1, metadata_1 = create_output_file(
            faker,
            user,
            PIPELINE_PAIR_OUTPUT_FILES,
            pair_name,
            request_id,
            assay,
...
def get_pairs(request):
    ...
    for single_file in files:
        ...
        if n_file_query:
            ...
            pooled_normal_query = FileRepository.filter(file_group=pooled_normal_group, metadata={"genePanel": assay})
...
def create_run_from_request(faker, user, num_requests):
    ...
    while current_request_num < num_requests:
        ...
        if num_failed > 0:
            ...
            end_date = faker.date_time_between_dates(datetime_start=RUN_START, datetime_end=RUN_END, tzinfo=EASTERN)
        ...
        operator_run = OperatorRun(
            ...
            operator=operator,
            num_total_runs=num_total,
            num_completed_runs=num_completed,
            num_failed_runs=num_failed,
            job_group=job_group,
...
TUMOR_ONCO_TREE_CODE = [
    ...
    "Acute Myeloid Leukemia (AML/LAML)",
    "ACUTE MYELOID LEUKEMIA (AML/LAML)",
    "ACYC",
    "ADNOS",
    "AGNG",
    ...
    "BLADDER UROTHELIAL CARCINOMA (BLCA)",
    "BLADDER_UROTHELIAL_CARCINOMA_(BLCA)",
    "BLCA",
    "BLCA)",
    "BLL",
    "BLLBCRABL1L",
    ...
    "EMPD",
    "EMPSGC",
    "ENDOMETRIOID OVARIAN CANCER (EOV)",
    "ENKL",
    "EOV",
    "EPDCA",
    "EPENDYMOMA",
    ...
    "LCH",
    "LCLC",
    "LENTIGO MALIGNA MELANOMA (SKLMM)",
    "LEUK",
    "LGESS",
    "LGGNOS",
    "LGNET",
    ...
    "NCCRCC",
    "NECNOS",
    "Neither OncoTree code nor name found for: Mixed Germ Cell Tumor(OMGCT)",
    "Neither OncoTree code nor name found for: Mucosal Melanoma of the Vulva/Vagina",
    "Neither OncoTree code nor name found for: Solitary Fibrous Tumor/Hemangiopericytoma",
    "NETNOS",
    "NFIB",
    "NHL",
    "NKCLL",
    ...
    "OVARIAN EPITHELIAL CARCINOMA (OVCA)",
    ...
    "PRNE",
    "PRNET",
    "PROLIFERATING PILAR CYSTIC TUMOR (PPCT)",
    "PROSTATE",
...
ASSAY_REFERENCE_FILES = [
    "{}_b37_baits.bed",
    "{}_b37_baits.ilist",
    "{}_b37_targets.bed",
    "{}_b37_targets.ilist",
    "{}_fingerprint_snps.vcf",
    "{}_FP_tiling_genotypes.txt",
    "{}_FP_tiling_intervals.intervals",
]
PIPELINE_PAIR_OUTPUT_FILES = [
    "{}.annotate-variants.vcf",
    "{}.combined-variants.vcf.gz",
    "{}.combined-variants.vcf.gz.tbi",
    "{}.muts.maf",
    "{}.svs.pass.vcf",
    "{}.svs.pass.vep.maf",
...
PIPELINE_SAMPLE_OUTPUT_FILES = [
    "{}.rg.md.asmetrics",
    "{}.rg.md_FP_base_counts.txt",
    "{}.rg.md.gcbiasmetrics",
    "{}.rg.md.gcbias.pdf",
    "{}.rg.md.gcbias.summary",
    "{}.rg.md.hsmetrics",
    "{}.rg.md.hstmetrics",
    "{}.rg.md.ismetrics",
    "{}.rg.md.ismetrics.pdf",
    "{}.rg.md_metrics",
    "{}.rg.md.pileup",
    "{}.rg.md.abra.printreads.bai",
    "{}.rg.md.abra.printreads.bam",
    "{}.rg.md.abra.printreads.qmetrics.qcycle_metrics",
    "{}.rg.md.abra.printreads.qmetrics.quality_by_cycle.pdf",
]
PIPELINE_QC_OUTPUT_FILES = [
    "{}_01_alignment.pdf",
    "{}_02_alignment_percentage.pdf",
    "{}_03_capture_specificity.pdf",
    "{}_04_capture_specificity_percentage.pdf",
    "{}_05_insert_size.pdf",
    "{}_06_insert_size_peaks.pdf",
    "{}_07_fingerprint.pdf",
    "{}_08_major_contamination.pdf",
    "{}_09_minor_contamination.pdf",
    "{}_11_duplication.pdf",
    "{}_12_library_size.pdf",
    "{}_13_coverage.pdf",
    "{}_15_base_qualities.pdf",
    "{}_16_gc_bias.pdf",
    "{}_18_minor_contam_freq_hist.pdf",
    "{}_contamination.R",
    "{}_contamination.txt",
    "{}_DiscordantHomAlleleFractions.txt",
    "{}_FingerprintSummary.txt",
    "{}_GcBiasMetrics.txt",
    "{}_HsMetrics.txt",
    "{}_InsertSizeMetrics_Histograms.txt",
    "{}_MajorContamination.txt",
    "{}_markDuplicatesMetrics.txt",
    "{}_MinorContamFreqList.txt",
    "{}_MinorContamination.txt",
    "{}_post_recal_MeanQualityByCycle.txt",
    "{}_pre_recal_MeanQualityByCycle.txt",
    "{}_ProjectSummary.txt",
    "{}_SampleSummary.txt",
    "{}_QC_Report.pdf",
    "qcPDF.log",
