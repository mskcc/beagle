...
if __name__=='__main__':
    URL = "http://jira.mskcc.org:8090/"
    USERNAME = "scruffy"
    PASSWORD = "piscruffysnackdrawer"
    PROJECT = "RSLDEV"
    jr = JiraClient(URL, USERNAME, PASSWORD, PROJECT)
    # Example create ticket
    # r = jr.create_ticket("TEST-TICKET", None, "TEST CREATING A JIRA TICKET")

    # Example search for ticket and return id
    ticket_number = jr.get_ticket("08944_B").json()['issues'][0]['key']

    # Example add comment to ticket
    # r = jr.comment(ticket_number, "TESTING ADDING COMMENTS TO TICKET\nMULTILINE TEST 2")

    # Example get transitions
    r = jr.get_status_transitions(ticket_number)
    print(r.json())
