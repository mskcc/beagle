...
from .serializers import (
    ...
    JobGroupQuerySerializer,
    CreateNotifierSerializer,
...
class JobGroupNotificationView(GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"details": "JobGroupNotifier %s Not Found" % job_notifier}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response({"details": "Event sent %s" % str(event)}, status=status.HTTP_201_CREATED)
...
class ProjectStatusView(mixins.ListModelMixin, GenericViewSet):
    queryset = (
        JobGroupNotifier.objects.filter(jira_id__startswith=settings.JIRA_PREFIX)
        .order_by("request_id", "-created_date")
        .distinct("request_id")
        .all()
    )
